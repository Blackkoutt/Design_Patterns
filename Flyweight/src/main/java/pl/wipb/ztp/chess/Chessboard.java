...
public class Chessboard extends JPanel {
	...
	private int mouseX = 0, mouseY=0; // Współrzędne X Y myszy
	...
	public void paintComponent(Graphics g) {
		...
		for (Map.Entry<Point, IPiece> e : board.entrySet()) {
			...
			pc.draw(g2d, point.x, point.y);
		}
		if (mouse != null && dragged != null) {
			dragged.draw(g2d, mouseX, mouseY);// Rysuj w aktualnych współrzędnych myszy
		}
	}
	...
	Chessboard() {
		...
		// Zamiast podawać współrzędne pobieramy tylko figure z tablicy
		board.put(new Point(0, 2), new Board(Piece.getPiece(11)));
		board.put(new Point(0, 6), new Board(Piece.getPiece(0)));
		board.put(new Point(1, 4), new Board(Piece.getPiece(6)));
		board.put(new Point(1, 5), new Board(Piece.getPiece(5)));
		board.put(new Point(3, 7), new Board(Piece.getPiece(1)));
		board.put(new Point(4, 3), new Board(Piece.getPiece(6)));
		board.put(new Point(4, 4), new Board(Piece.getPiece(7)));
		board.put(new Point(5, 4), new Board(Piece.getPiece(6)));
		board.put(new Point(5, 6), new Board(Piece.getPiece(0)));
		board.put(new Point(6, 5), new Board(Piece.getPiece(0)));
		board.put(new Point(7, 4), new Board(Piece.getPiece(0)));
		...
		this.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent ev) {
				// Aktualizujemy współrzędne muszy
				mouseX = (ev.getX() - ZEROX) / Piece.TILESIZE;
				mouseY = (ev.getY() - ZEROY) / Piece.TILESIZE;
				dragged = new Moving(take(mouseX,mouseY ));
				mouse = ev.getPoint();
			}
